...
# Context managers allow us to allocate and release resources precisely when we want to for the view of web
# application
...
# when the user wants to change the model other than the default model and train the choosen model
...
# getting crypto symbols from the binance api
...
# defining styles and content for the HOME page
def main():
    ...
    if radio == "Home":
        ...
        with col1:
            st.subheader('Features of our project:')
            st.markdown(
                """ 
                * 📂 Generate cryptocurrency price data.\n
                * 📊 Data visualization.\n
                * 📈 Predict cryptocurrency price with neural networks model (ANN).\n
                * ⚙️ Training model and Hyperparameters tuning.\n
                """)
        st.markdown('---')
        st.subheader('Team ')
        st.markdown(
        ...
        if page == 'Training':
            ...
            if os.path.isfile(f'./model/{symbol}_model_{col}.h5'):
                st.info(f'{symbol} {col} price is kept as default model for this web app.The model can be re-trained\
                    by changing the hyperparameters and clicking\
                    the \'Train Model\'  below.')
            ...
            try:
                ...
                with st.expander('ANN Architecture'):
